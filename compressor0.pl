...
/* increment(2, 6, L, R).  L = [3, 4, 5, 6, 7, 8|R] . 
    ...
    w for the existence of a queue R in the list
*/
increment(X, 0, [], _).
increment(X, Times, [Y|Result], Rest) :-
    plusOne(X, Y),
    increment(Y, Times1, Result, Rest),
    Times is Times1+1.
...
testCompress :-
    ...
    complexity(CL, CCL, _),
        write('compressed version 2:\t'), write(CCL), nl,

    compress(L1, CCL),  % Funny: 'compress' is used to decompress
    !,  % necessary, since reversibility is not perfect
        write('recovered list:  \t'), write(L1), nl,
    compress(L2, L1),  % Funny: 'compress' is used to decompress
    !,  % necessary, since reversibility is not perfect
        write('recovered list 2:  \t'), write(L2), nl.
